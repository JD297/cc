...
int main()
{
    ...
    assert(str_list_push_back(source_files, "tests/assets/asset_test_line_macro.c") != -1);
    ...
    assert(token_list_c_push_back(list, test_token_c_create(T_INT, "1")) == 0);
    assert(token_list_c_push_back(list, test_token_c_create(T_WHITESPACE_LINE_FEED, "\n")) == 0);
    assert(token_list_c_push_back(list, test_token_c_create(T_INT, "7")) == 0);
