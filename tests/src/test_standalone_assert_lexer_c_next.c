...
int main()
{
	...
	Token_C asset_test_tokens[ASSET_TEST_C_TOKEN_NUM] = {
		...
		{T_INT, "int"}, {T_IDENTIFIER, "main"}, {T_OPEN_PARENT, "("}, {T_CLOSING_PARENT, ")"}, {T_WHITESPACE_LINE_FEED, "\n"},			// LINE 3
	...
	for (size_t i = 0; i < ASSET_TEST_C_TOKEN_NUM; i++) {
		...
		if (!lexer_c_token_type_is_in_expected_token_types(asset_test_tokens[i].type, TOKEN_TYPE_C_WITH_NO_REPRESENTATION_NUM, TOKEN_TYPE_C_WITH_NO_REPRESENTATION)) {
			assert(strcmp(token->value, asset_test_tokens[i].value) == 0);
		} else {
			assert(strcmp(token->value, lexer_c_token_type_representation(asset_test_tokens[i].type)) == 0);
		}
